...
import { getContactInvoices, selectInvoicesRange, setInvoicesLimit, setInvoicesOrder, setInvoicesPage } from 'src/slices/contactDetail';
import MobileDatePicker from '@mui/lab/MobileDatePicker';
import moment from 'moment/moment';
import InvoiceStatusLabel from 'src/components/widgets/invoice/InvoiceStatusLabel';
import { useRouter } from 'next/router';

const ContactInvoices: FC = (props) => {
  const router = useRouter();
  const { contactId } = router.query;
  const { organisation } = useSelector((state) => state.account);
  const { invoices, selectedInvoicesRange, isInvoicesLoading, invoicesTotal, invoicesPage, invoicesLimit, invoicesOrder, invoicesOrderBy } = useSelector((state) => state.contactDetail);
  const dispatch = useDispatch();

  useEffect(() => {
    // @ts-ignore
    dispatch(getContactInvoices(organisation.id, parseInt(contactId, 10), selectedInvoicesRange, invoicesLimit, invoicesPage, invoicesOrderBy));
  }, [organisation, getContactInvoices, contactId, selectedInvoicesRange, invoicesLimit, invoicesPage, invoicesOrderBy]);
