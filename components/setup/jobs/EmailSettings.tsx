...
const EmailSettings: FC = (props) => {
  ...
  const invoiceMergeTags = map(values(INVOICE_MERGE_TAGS), (tag) => ({ name: tag }));

  return (
    <Formik
      enableReinitialize
      initialValues={{
        reply_to: invoiceSettings.reply_to || '',
        cc_email: invoiceSettings.cc_email || '',
        subject_message: invoiceSettings.subject_message || '',
        intro_message: invoiceSettings.intro_message || '',
        submit: null
      }}
      validationSchema={
        Yup
      ...
      {({
        ...
        >
          <Card {...props}>
            ...
            <Divider />
            <CardContent>
              <MergeTagsInfo mergeTags={invoiceMergeTags} />
              <Grid
                container
                spacing={2}
                sx={{
